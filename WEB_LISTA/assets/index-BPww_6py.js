const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/LandingPage-BPN4EoB_.js","assets/dbServices-DWcSe7j2.js","assets/map-pin-BDAxippK.js","assets/qr-code-BN-YDb3n.js","assets/globe-BmAyi19K.js","assets/search-U-GGTmJ3.js","assets/zap-2RP5tIvg.js","assets/LoginPage-C98aSV_b.js","assets/user-_02PLGKS.js","assets/building-2-BjZvvZN7.js","assets/eye-off-BnmzXk71.js","assets/eye-B7vgoIDk.js","assets/circle-help-DHeMaGPA.js","assets/ResetPassword-Ck5sRVia.js","assets/MasterDashboard-B78cGABN.js","assets/planes-CbXPniKB.js","assets/TileLayer-BD85jWJ5.js","assets/users-CfsZdzkk.js","assets/log-out-1pdGr_bw.js","assets/wifi-B1EFBYcL.js","assets/MasterDashboard-CIGW-MKW.css","assets/SupportDashboard-Bw-BrCEo.js","assets/pen-line-CyofpdCC.js","assets/clock-DYWozcSp.js","assets/CompanyDashboard-Bg9_RRYy.js","assets/play-DWixwAmZ.js","assets/StaffApp-BfNBLVhk.js"])))=>i.map(i=>d[i]);
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */xi(ZD,eO,"app");let Bo;console.warn("Firebase not configured. Entering DEMO MODE."),Bo=null;const vt=Bo?CP(Bo):{isMock:!0,onAuthStateChanged:t=>(setTimeout(()=>t(null),100),()=>{})},Hy=Bo?$D(Bo):null;new dn;const R0=L.createContext(),ae={SUPER_ADMIN:"SUPER_ADMIN",SUPPORT:"SUPPORT",COMPANY_ADMIN:"COMPANY_ADMIN",OPERADOR:"OPERADOR",SUPERVISOR:"SUPERVISOR",GUARD:"GUARD"},tO=({children:t})=>{const[e,n]=L.useState(null),[r,i]=L.useState(!0),s=async h=>{if(!h||!Hy)return null;try{const f=zD(Hy,"usuarios",h.uid),p=await YD(f);if(p.exists()){const m=p.data();return m.activo===!1?(await Dg(vt),null):{uid:h.uid,...m}}return null}catch{return null}};L.useEffect(()=>{try{const f=localStorage.getItem("centinela_current_user");if(f){n(JSON.parse(f)),i(!1);return}}catch(f){console.warn("Auth restore error:",f),localStorage.removeItem("centinela_current_user")}let h;return vt!=null&&vt.isMock?(setTimeout(()=>i(!1),500),h=()=>{}):h=mR(vt,async f=>{if(i(!0),f){const p=await s(f);n(p)}else n(null);i(!1)}),()=>h()},[]),L.useEffect(()=>{if(vt!=null&&vt.isMock){const h=JSON.parse(localStorage.getItem("centinela_companies")||"[]");if(!h.find(p=>p.id==="demo_001")){const p={id:"demo_001",name:"Empresa de Seguridad Demo",nombre:"Empresa de Seguridad Demo",titular:"Admin Demo",email:"admin@empresa.com",plan:"demo",status:"activa",lat:-34.6037,lng:-58.3816,address:"Av. Corrientes 1234, CABA",fecha_alta:new Date().toISOString().split("T")[0],expiryDate:"2026-12-31"};localStorage.setItem("centinela_companies",JSON.stringify([...h,p]))}const f=JSON.parse(localStorage.getItem("centinela_users")||"[]");if(!f.find(p=>p.email==="admin@empresa.com")){const p={id:"db_demo_admin",nombre:"Admin",apellido:"Demo",email:"admin@empresa.com",password:"123456",rol:ae.COMPANY_ADMIN,empresaId:"demo_001",activo:!0};localStorage.setItem("centinela_users",JSON.stringify([...f,p]))}}},[]);const o=async(h,f)=>{const p=h.toLowerCase().trim();try{const{loginRemoto:m}=await rn(async()=>{const{loginRemoto:R}=await import("./dbServices-DWcSe7j2.js");return{loginRemoto:R}},[]),A=await m(p,f);if(A&&A.success&&A.user){const R=A.user;let N=(R.role||R.rol||ae.GUARD).toUpperCase();if(p.toLowerCase()==="vidal@master.com")N=ae.SUPER_ADMIN;else{const S=N;S.includes("SUPER")&&S.includes("ADMIN")?N=ae.SUPER_ADMIN:S.includes("SOPORTE")||S.includes("SUPPORT")?N=ae.SUPPORT:S.includes("ADMIN")?N=ae.COMPANY_ADMIN:S.includes("OPERADOR")||S.includes("OPERARIO")||S.includes("MONITOREO")?N=ae.OPERADOR:S.includes("SUPERVISOR")||S==="SUP"?N=ae.SUPERVISOR:(S.includes("GUARD")||S.includes("VIGILADOR")||S.includes("SERENO"))&&(N=ae.GUARD)}const O={...R,rol:N,empresaId:R.companyId||R.empresaId,nombre:R.name||R.nombre,mustChangePassword:R.password==="123456"||R.password==="password123"||!R.password};return n(O),localStorage.setItem("centinela_current_user",JSON.stringify(O)),O}}catch(m){console.warn("Remote Login Failed, trying local/emergency fallback:",m)}if(p==="pablo@stark.com"&&f==="password123"){const m={uid:"p_001",nombre:"Pablo Stark",email:"pablo@stark.com",rol:ae.GUARD,mustChangePassword:!0};return n(m),localStorage.setItem("centinela_current_user",JSON.stringify(m)),m}if(p==="vidal@master.com"||p==="master")try{const{verificarAdmin:m}=await rn(async()=>{const{verificarAdmin:N}=await import("./dbServices-DWcSe7j2.js");return{verificarAdmin:N}},[]),A=await m(f),R=f==="123456"||f==="admin";if(A!=null&&A.success||!A&&R){const N={uid:"master_001",nombre:"Vidal (Super Admin)",email:"vidal@master.com",rol:ae.SUPER_ADMIN,estado:"Activo"};return n(N),localStorage.setItem("centinela_current_user",JSON.stringify(N)),N}}catch(m){if(console.error("Auth Emergency Fallback:",m),f==="123456"||f==="admin"){const A={uid:"master_001",nombre:"Vidal (Super Admin)",email:"vidal@master.com",rol:ae.SUPER_ADMIN,estado:"Activo"};return n(A),localStorage.setItem("centinela_current_user",JSON.stringify(A)),A}}if(f==="123456"||f==="admin"){let m=null;if(p==="soporte@centinela.com"||p==="soporte"?m={uid:"support_001",nombre:"Centinela Staff (Soporte)",email:"soporte@centinela.com",rol:ae.SUPPORT,estado:"Activo"}:(p==="admin@empresa.com"||p==="admin")&&(m={uid:"admin_demo",nombre:"Administrador Demo",email:"admin@empresa.com",rol:ae.COMPANY_ADMIN,empresaId:"demo_001",estado:"Activo",mustChangePassword:!1}),m)return n(m),localStorage.setItem("centinela_current_user",JSON.stringify(m)),m}if(vt&&!vt.isMock)try{const m=await dR(vt,p,f),A=await s(m.user);return A&&localStorage.setItem("centinela_current_user",JSON.stringify(A)),A}catch(m){throw m}else throw new Error("CREDENTIALS_MISMATCH: Verifique su usuario y contraseña.")},l=async h=>{try{if(!e)throw console.error("[AUTH] No user in state for password update"),new Error("Sesión no válida. Cierre sesión e intente nuevamente.");const{apiRequest:f}=await rn(async()=>{const{apiRequest:R}=await import("./dbServices-DWcSe7j2.js");return{apiRequest:R}},[]),p={userId:e.uid||e.id,email:e.email||e.appEmail,newPassword:h};console.log("[AUTH] Syncing password change:",{...p,newPassword:"***"});const m=await f("/usuarios/update-password","POST",p);if(!m||m.success!==!0)throw new Error((m==null?void 0:m.error)||"El servidor no confirmó el cambio.");console.log("[AUTH] Password synchronized with server.");const A={...e,mustChangePassword:!1,password:h};return n(A),localStorage.setItem("centinela_current_user",JSON.stringify(A)),!0}catch(f){throw console.error("[AUTH] Update password error:",f.message),f}},u=async()=>{i(!0);try{vt.isMock||await Dg(vt),n(null),localStorage.removeItem("centinela_current_user")}catch{n(null),localStorage.removeItem("centinela_current_user")}finally{i(!1)}};return V.createElement(R0.Provider,{value:{user:e,login:o,logout:u,updatePasswordDemo:l,loading:r,ROLES:ae}},t)},ku=()=>{const t=L.useContext(R0);if(!t)throw new Error("useAuth must be used within an AuthProvider");return t},P0=L.createContext(),ZO=()=>{const t=L.useContext(P0);if(!t)throw new Error("useSound must be used within a SoundProvider");return t},nO=({children:t})=>{const{user:e}=ku(),[n,r]=L.useState({enabled:!0,generalVolume:.5,panicVolume:.8}),[i,s]=L.useState(!1),[o,l]=L.useState(!1),[u,h]=L.useState(""),[f,p]=L.useState(!1),m=L.useRef(null),A=L.useRef(null),R=L.useRef(null);L.useEffect(()=>{try{m.current=new Audio("/sounds/normal.mp3"),A.current=new Audio("/sounds/qr.mp3"),R.current=new Audio("/sounds/panico.mp3"),R.current.loop=!1}catch(_){console.warn("Audio Context restricted by browser:",_)}if(e!=null&&e.uid)try{const _=localStorage.getItem(`centinela_sound_settings_${e.uid}`);_&&r(JSON.parse(_))}catch(_){console.warn("Sound settings restore failed:",_)}},[e==null?void 0:e.uid]),L.useEffect(()=>{m.current&&(m.current.volume=n.generalVolume),A.current&&(A.current.volume=n.generalVolume),R.current&&(R.current.volume=n.panicVolume)},[n.generalVolume,n.panicVolume]);const N=_=>{const E={...n,..._};r(E),e!=null&&e.uid&&localStorage.setItem(`centinela_sound_settings_${e.uid}`,JSON.stringify(E))},O=()=>{[m,A,R].forEach(_=>{_.current&&(_.current.pause(),_.current.currentTime=0)}),s(!1),l(!1),h(""),window.speechSynthesis&&window.speechSynthesis.cancel()},S=()=>{R.current&&(R.current.pause(),R.current.currentTime=0),s(!1),o||h(""),window.speechSynthesis&&window.speechSynthesis.cancel()},v=()=>{l(!1),i||h(""),window.speechSynthesis&&window.speechSynthesis.cancel()},C=_=>{try{const E=window.AudioContext||window.webkitAudioContext;if(!E)return;const T=new E,P=(I,oe,Pe,yt,xt="sine",$=!1)=>{const q=T.createOscillator(),Q=T.createGain();q.type=xt,q.connect(Q),Q.connect(T.destination),q.frequency.setValueAtTime(I,T.currentTime+oe),$&&q.frequency.exponentialRampToValueAtTime(I*1.5,T.currentTime+oe+Pe),Q.gain.setValueAtTime(0,T.currentTime+oe),Q.gain.linearRampToValueAtTime(yt*.2,T.currentTime+oe+.05),Q.gain.exponentialRampToValueAtTime(.01,T.currentTime+oe+Pe),q.start(T.currentTime+oe),q.stop(T.currentTime+oe+Pe)};if(_==="normal")P(440,0,.5,n.generalVolume),P(660,.6,.8,n.generalVolume);else if(_==="qr")for(let I=0;I<5;I++)P(800+I*100,I*.3,.2,n.generalVolume,"sine",!0);else if(_==="missed_round")for(let I=0;I<3;I++)P(500,I*.4,.2,n.panicVolume,"triangle");else if(_==="panico"){const oe=T.createOscillator(),Pe=T.createGain(),yt=T.createOscillator(),xt=T.createGain();oe.type="square",oe.connect(Pe),Pe.connect(T.destination),yt.type="sine",yt.frequency.setValueAtTime(.5,T.currentTime),xt.gain.setValueAtTime(200,T.currentTime),yt.connect(xt),xt.connect(oe.frequency),oe.frequency.setValueAtTime(600,T.currentTime),Pe.gain.setValueAtTime(0,T.currentTime),Pe.gain.linearRampToValueAtTime(n.panicVolume*.3,T.currentTime+.1),Pe.gain.setValueAtTime(n.panicVolume*.3,T.currentTime+5-.5),Pe.gain.exponentialRampToValueAtTime(.01,T.currentTime+5),yt.start(),oe.start(),yt.stop(T.currentTime+5),oe.stop(T.currentTime+5)}}catch(E){console.error("Synth fallback failed",E)}},x=L.useRef(null),M=_=>{if(!window.speechSynthesis)return;window.speechSynthesis.cancel();const E=new SpeechSynthesisUtterance(_);return E.lang="es-AR",E.rate=.9,E.pitch=1,x.current=E,window.speechSynthesis.speak(E),E},F=async(_,E="")=>{if(!n.enabled&&_!=="panico"&&_!=="missed_round")return;if(_==="panico"){O(),s(!0),E&&h(E);return}if(_==="missed_round"){O(),l(!0),E&&h(E);return}if(i||o)return;let T=null;if(_==="normal"&&(T=m.current),_==="qr"&&(T=A.current),T){if(E&&(M(E),await new Promise(P=>setTimeout(P,800))),!T.paused&&T.currentTime>0&&T.currentTime<.2)return;try{T.currentTime=0,await T.play(),p(!1)}catch(P){P.name==="NotAllowedError"?p(!0):C(_)}}},w=_=>{let E=null;_==="normal"&&(E=m.current),_==="qr"&&(E=A.current),_==="panico"&&(E=R.current),E?(E.currentTime=0,E.play().then(()=>{p(!1)}).catch(T=>{T.name==="NotAllowedError"?p(!0):(C(_),p(!1))})):(C(_),p(!1))};L.useEffect(()=>{let _=null,E=!1;const T=async()=>{if(!E&&(i||o)){if(M(u||(o?"Atención: Ronda no realizada":"Alerta: Botón de pánico activado")),await new Promise(I=>_=setTimeout(I,3500)),E||!i&&!o)return;if((i?"panico":"missed_round")==="panico")try{await R.current.play(),p(!1)}catch{C("panico")}else C("missed_round");if(await new Promise(I=>_=setTimeout(I,6e3)),E||!i&&!o)return;T()}};return(i||o)&&T(),()=>{E=!0,_&&clearTimeout(_)}},[i,o,u]);const y={settings:n,saveSettings:N,playSound:F,stopPanic:S,stopMissedRound:v,speak:M,isPanicActive:i,isMissedRoundActive:o,autoplayBlocked:f,setAutoplayBlocked:p,testSound:w};return V.createElement(P0.Provider,{value:y},t)};/**
