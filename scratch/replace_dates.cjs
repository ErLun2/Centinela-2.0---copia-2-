...
let occurrences1 = content.split("new Date().toISOString().split('T')[0]").length - 1;
content = content.split("new Date().toISOString().split('T')[0]").join("getARDateStr(new Date())");

let occurrences2 = content.split("e.fechaRegistro.split('T')[0]").length - 1;
content = content.split("e.fechaRegistro.split('T')[0]").join("getARDateStr(e.fechaRegistro)");

let occurrences3 = content.split("mediaModal.event?.fechaRegistro?.split('T')[0]").length - 1;
...
console.log(`Replaced ${occurrences1} calls of new Date()...`);
console.log(`Replaced ${occurrences2} calls of e.fechaRegistro...`);
console.log(`Replaced ${occurrences3} calls of mediaModal...`);
